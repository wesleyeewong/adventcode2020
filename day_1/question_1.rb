...
def solve_part_1(entries)
  ...
  entries.each do |entry|
    remaining = (entry - answer).abs

    if set.include?(remaining)
      p entry * remaining
    end
...
def solve_part_2(entries)
  ...
  entries.each do |entry|
    entries.each do |entry_2|
      remaining = answer - (entry + entry_2)

      if remaining.positive? && set.include?(remaining)
        p "#{entry} and #{entry_2} and #{remaining}"
        p entry * entry_2 * remaining
      end
    end
